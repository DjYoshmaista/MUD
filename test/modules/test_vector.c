...
TEST(vector_create_destroy) {
    ...
    LOG_TEST_TRACE("Verifying initial state");
...
TEST(vector_null_safety) {
    ...
    LOG_TEST_TRACE("Testing read operations with NULL");
    ...
    LOG_TEST_TRACE("Testing write operations with NULL");
    ...
    LOG_TEST_TRACE("Testing destroy with NULL");
...
TEST(vector_push_get_set) {
    ...
    LOG_TEST_TRACE("Size: %zu, Capacity: %zu", mud_vector_size(vec), mud_vector_capacity(vec)); 
    ...
    LOG_TEST_TRACE("Testing out-of-bounds set (should fail)");
...
TEST(vector_insert_pop_remove) {
    ...
    for (size_t i = 0; i <= 3; i++) {
        LOG_TEST_TRACE("Vector[%zu]: %d", i, *(int*)mud_vector_get(vec, i));
    }
    ...
    LOG_TEST_TRACE("Popped value: %d", popped);
    for (size_t i = 0; i <= 1; i++) {
        LOG_TEST_TRACE("Vector[%zu]: %d", i, *(int*)mud_vector_get(vec, i));
    }

    LOG_TEST_TRACE("Testing invalid remove (should fail)");
...
TEST(vector_clear_reserve_shrink) {
    ...
    LOG_TEST_TRACE("Capacity after reserve: %zu", mud_vector_capacity(vec));
    ...
    LOG_TEST_TRACE("Capacity after shrink: %zu", mud_vector_capacity(vec));

    TEST_LOG_DEBUG("Clearing vector");
    mud_vector_clear(vec);
    CHECK(mud_vector_is_empty(vec));
    CHECK_INT_EQ(ctx, mud_vector_capacity(vec), 12);
    LOG_TEST_TRACE("Capacity preserved after clear: %zu", mud_vector_capacity(vec));
...
TEST(vector_data_access) {
    ...
    for (size_t i = 0; i < 5; i++) {
        CHECK(mud_vector_push(vec, &i));
        LOG_TEST_TRACE("Vector[%zu] checked: %d", i + 1, *(int*)mud_vector_get(vec, i));
    }
    ...
    LOG_TEST_TRACE("Verifying const accessor returns same address");
...
TEST(vector_type_safe_macros) {
    ...
    LOG_TEST_TRACE("Index 2 (via ptr): %f (expected ~1.41421)", *dbl_ptr);
