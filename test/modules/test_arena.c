...
TEST(arena_create_destroy) {
    ...
    TEST_LOG_INFO("Arena capacity: %zu", mud_arena_capacity(arena));
    CHECK_INT_EQ(ctx, mud_arena_capacity(arena), 1024);
    CHECK_INT_EQ(ctx, mud_arena_used(arena), 0);
    TEST_LOG_INFO("Arena used: %zu", mud_arena_used(arena));
    CHECK_INT_EQ(ctx, mud_arena_remaining(arena), 1024);
    TEST_LOG_INFO("Arena remaining: %zu", mud_arena_remaining(arena));
...
TEST(arena_null_safety) {
    ...
    TEST_LOG_INFO("===Initial Arena Statistics===\n  - Arena Capacity: %zu\n  - Arena Used: %zu\n  - Arena Remaining: %zu", mud_arena_capacity(NULL), mud_arena_used(NULL), mud_arena_remaining(NULL));
...
TEST(arena_alloc_zero) {
    ...
    for (int i = 0; i < 64; i++) {
        if (i % 8 == 0) LOG_TEST_TRACE("Checking byte %d of data", i);
        CHECK_INT_EQ(ctx, data[i], 0);
        if (i % 8 == 0) LOG_TEST_TRACE("Byte %d of data successfully checked", i);
    }
...
TEST(arena_alignment_default) {
    ...
    for (int i = 0; i < 10; i++) {
        if (i % 10 == 0) LOG_TEST_TRACE("Checking alignment of allocation of size %d", i);
        size_t size = (size_t)(i * 7 + 1); // Varying sizes: 1, 8, 15, 22, ...
        if (i % 10 == 0) LOG_TEST_TRACE("Allocating %zu bytes of memory to ptr", size);
        void* ptr = mud_arena_alloc(arena, size);

        REQUIRE_NOT_NULL(ctx, ptr);
        if (ctx->abort_current_test) { mud_arena_destroy(arena); return; }

        if (i % 10 == 0) LOG_TEST_TRACE("ptr successfully tested not NULL\nChecking alignment of ptr...");
        // Check alignment
        uintptr_t addr = (uintptr_t)ptr;
        CHECK_INT_EQ(ctx, addr % MUD_ARENA_DEFAULT_ALIGN, 0);
        if (i % 10 == 0) LOG_TEST_TRACE("ptr successfully checked alignment");
    }
...
TEST(arena_alignment_custom) {
    ...
    for (size_t i = 0; i < num_alignments; i++) {
        ...
        if (i % 3 == 0) LOG_TEST_TRACE("Checking 32-byte allocation with alignment %zu: ", align);

        void* ptr = mud_arena_alloc_aligned((MudArena*)arena, 32, align);

        if (i % 3 == 0) LOG_TEST_TRACE("Allocation attempted");
        ...
        if (i % 3 == 0) LOG_TEST_TRACE("ptr successfully passed alignment check for alignment %zu", align);
    }
...
TEST(arena_temp_basic) {
    ...
    TEST_LOG_DEBUG("arena_temp_basic: Temporary Scope Allocated Successfully.  Values: Arena - %p :: persistent - %s :: used_after_persistent - %zu :: temporary %s", (void*)arena, persistent, used_after_persistent, temporary);
...
TEST(arena_temp_nested) {
    ...
    LOG_TEST_TRACE("Inner Temporary Scope Ended");
