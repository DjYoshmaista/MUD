...
static bool checked_allocation_size(size_t count, size_t element_size, size_t* out) {
...
MudVector* mud_vector_create(size_t element_size) {
    ...
    size_t alloc_size = 0;
    if (!checked_allocation_size(MUD_VECTOR_INITIAL_CAPACITY, element_size, &alloc_size)) {
        free(vec);
        return NULL;
    }
...
static bool vector_grow(MudVector* vec) {
    ...
    if (!checked_allocation_size(new_capacity, vec->element_size, &alloc_size)) {
        return false;
    }
...
bool mud_vector_reserve(MudVector* vec, size_t min_capacity) {
    ...
    size_t alloc_size = 0;
    if (!checked_allocation_size(min_capacity, vec->element_size, &alloc_size)) {
        return false;
    }
...
bool mud_vector_shrink_to_fit(MudVector* vec) {
    ...
    if (vec->size == 0) {
		free(vec->data);
        size_t alloc_size = 0;
        if (!checked_allocation_size(MUD_VECTOR_INITIAL_CAPACITY, vec->element_size, &alloc_size)) {
            vec->data = NULL;
            vec->capacity = 0;
            return false;
        }
    ...
    size_t alloc_size = 0;
    if (!checked_allocation_size(vec->size, vec->element_size, &alloc_size)) {
        return false;
    }
